...
def guardar_fotos_avance(codigo_obra: str, fotos, fecha_hoy: date) -> List[str]:
    ...
    Si Cloudinary no está configurado, guarda localmente como fallback
    ...
    if configurar_cloudinary():
        try:
            ...
            if urls_cloudinary and len(urls_cloudinary) > 0:
                print(f"✅ {len(urls_cloudinary)} fotos subidas a Cloudinary")
                return urls_cloudinary
            else:
                print("⚠️ No se pudieron subir fotos a Cloudinary, usando almacenamiento local")
        except Exception as e:
            print(f"⚠️ Error con Cloudinary: {e}, usando almacenamiento local")
    else:
        print("ℹ️ Cloudinary no configurado, usando almacenamiento local")
...
def construir_curva_s_planificada(cronograma: List[Dict[str, Any]], freq: str = "Semanal") -> pd.DataFrame:
    ...
    elif f in ["D", "d"] or f_low.startswith("dia") or f_low.startswith("dí"):
        df["bucket"] = df["fecha"]
    else:
...
def validar_donacion(nombre_donante: str, tipo_donacion: str, monto_o_cantidad: Any, valor_unitario: Any = None) -> Tuple[bool, str]:
    """Valida los datos de una donación."""
    ...
    if not tipo_donacion or not str(tipo_donacion).strip():
        return False, "Debes seleccionar el tipo de donación (Efectivo/Insumo)"
    ...
    try:
        ...
        return False, "La cantidad o monto debe ser un número válido"
    ...
    if tipo_donacion.lower() == "insumo":
        ...
        try:
            ...
            return False, "El valor unitario debe ser un número válido"
...
def calcular_valor_donacion(tipo_donacion: str, cantidad: float, valor_unitario: float = None) -> float:
    """
    Calcula el valor total de la donación.
